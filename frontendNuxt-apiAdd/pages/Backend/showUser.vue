<template>
  ...
  <div class=" flex flex-col py-12 px-4">
    ...
    <h2 class="text-4xl text-red-500 font-bold text-center">Show All User Data</h2>
    ...
    <div class="flex justify-between">
      ...
      <form-button class="bg-red-600" :loading="loading" v-if="selected_data.length > 0"
        @click.prevent="delete_multiple()">Delete Selected ( {{ selected_data.length }} )</form-button>
      ...
      <nuxt-link to="/Backend/addUser" class=" bg-green-500 text-white py-1 px-8">+ Add User</nuxt-link>
            ...
            <table class="table-auto catList">
              ...
              <tbody class="bg-green-50 p-2 ">
                ...
                <tr v-for="(slid, index) in cat_list.data" :key="slid.id" class="border border-b border-green-200 ">
                  <td class="table-td">
                    <input v-if="selected_data.includes(slid.id)" checked type="checkbox"
                      @change="add_to_selected(slid.id)" class="form-check">
                  ...
                  <td>
                    <div class="flex my-1 lg:justify-between px-2">
                      <nuxt-link :to="`/backend/editUser/?id=${slid.id}`"
                        class="bg-yellow-600 py-1 px-2 mx-2 rounded text-center text-white ">Edit</nuxt-link>
                      <form-button @click.prevent="delete_slid(slid,index)" class="bg-red-600 text-white "
                        :loading="loading">Delete</form-button>
                    </div>
                  </td>
                </tr>
  ...
  export default {
    ...
    methods: {
      ...
      async delete_slid(slid, index) {
        ...
        if (con) {
          await this.$axios.$post('/api/user/s_admin/delete', {
            id: slid.id
          })
      ...
      async delete_multiple() {
        ...
        if (con) {
          this.loading = true;
          await this.$axios.$post('/api/user/s_admin/delete-multi', {
            ids: this.selected_data
          })
          this.selected_data = [];
          // this.cat_list.data.splice(index,1);
          this.getData(); // for show only 5 data
